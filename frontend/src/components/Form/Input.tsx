import React, { useReducer, useState, ChangeEvent, JSX } from "react";
...
export const Input = ({
  ...
  frameClassName,
  text = "First Name",
  // CHANGED: Using SearchRounded as the default placeholder icon
  icon = <SearchRounded className="!relative !w-[18px] !h-[18px] text-gray-500" />,
  divClassName,
  onChange,
  type = "text",
  name,
    ...
    >
      ...
      >
        ...
        {showRightIcon && (
          type === "password" ? (
            showPassword ? (
              // CHANGED: Replaced InputIconsEye with VisibilityOffRounded (icon to hide password)
              <VisibilityOffRounded 
                className="!relative !w-[18px] !h-[18px] text-gray-500 hover:text-gray-700 cursor-pointer" 
                onClick={togglePasswordVisibility} 
              />
            ) : (
              // CHANGED: Replaced InputIcons3 with VisibilityRounded (icon to show password)
              <VisibilityRounded 
                className="!relative !w-[18px] !h-[18px] text-gray-500 hover:text-gray-700 cursor-pointer" 
                onClick={togglePasswordVisibility} 
              />
            ...
            // CHANGED: Replaced generic InputIcons3 with SearchRounded
            <SearchRounded className="!relative !w-[18px] !h-[18px] text-gray-500" />
          )
        )}
      </div>
